...
### Step 4 — 轉移 `routine-prompt.md` (`.gitignored`，git 不會帶)

`routine-prompt.md` 是給 Claude 讀的 routine 指令；當初設定為本地檔不上傳。從舊 Mac 拷貝：

```bash
# 從舊 Mac (假設可 ssh 進去)
scp old-mac:~/Documents/Sean/project/us-premarket-brief/routine-prompt.md ~/Documents/Sean/project/us-premarket-brief/

# 或：iCloud / Dropbox / dotfiles 同步
```

**或者**：解除 `.gitignore` 限制讓它跟著 git 走 (要評估是否在乎內容上傳)：
```bash
# 在 .gitignore 移除 routine-prompt.md 那行，commit 一次
```
...
| 改 prompt (本機 only) | 編 `~/Documents/Sean/project/us-premarket-brief/routine-prompt.md` |
...
| `.gitignore` | 忽略 `routine-prompt.md` |
| `README.md` | 本文件 |
...
| 路徑 | 用途 |
|---|---|
| `routine-prompt.md` | 給 Claude 讀的 routine 指令；本機 only |
